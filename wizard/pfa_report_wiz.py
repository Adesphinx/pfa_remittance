...
class payrollreportexcelwiz(models.TransientModel):
    ...
    def get_rules(self):
        ...
        heads = self.env['hr.salary.rule'].search([('active', 'in', (True, False)), ('code', 'in', ['CRP', 'COMP', 'TRP'])])
        list = []
        for head in heads:
    ...
    def get_item_data(self):
        file_name = _('PFA remittance report.xlsx')
        fp = BytesIO()
        ...
        worksheet = workbook.add_worksheet('PFA remittance report.xlsx')
        ...
        if self.from_date and self.date_end:
            ...
            worksheet.merge_range('A1:E2', 'Bank Remittance For %s %s' % (payroll_month, self.from_date.year), heading_format)
            ...
            worksheet.write(row, 4, date_2 or '')
            row += 2
            res=self.get_rules()

            worksheet.write(row, 0, 'Employee', cell_text_format)
            worksheet.write(row, 1, 'Employee ID', cell_text_format)
            worksheet.write(row, 2, 'PFA Name', cell_text_format)
            worksheet.write(row, 3, 'RSA Number(PIN)', cell_text_format)
            ...
            if payslip_ids:
                ...
                for payslip in payslip_ids:
                    ...
                    pfa_name_id = payslip.employee_id.pfa_name
                    pfa_no_id = payslip.employee_id.pfa_no



                    worksheet.write(ro, col, name or '', cell_text_format_new)
                    worksheet.write(ro, col + 1, id or '', cell_text_format_new)
                    worksheet.write(ro, col + 2, pfa_name_id or '', cell_text_format_new)
                    worksheet.write(ro, col + 3, pfa_no_id or '', cell_text_format_new)
        ...
        return {
            'name': 'PFA remittance report Download',
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'pfa.report.excel',
...
class payroll_report_excel(models.TransientModel):
    _name = 'pfa.report.excel'

    name = fields.Char('File Name', size=256, readonly=True)
    file_download = fields.Binary('Download PFA Remittance', readonly=True)
